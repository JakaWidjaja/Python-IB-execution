...
class BlackScholes:
    ...
    def Price(self, stock, strike, vol, expiry, rate, optType, dividendRate = 0.0):
        self.stock        = stock
        self.strike       = strike
        self.vol          = vol
        self.expiry       = expiry
        ...
        discount = np.exp(-self.expiry * (self.rate - self.dividendRate))
        
        d1 = (np.log(self.stock / self.strike) + self.expiry * ((self.rate - self.dividendRate) + 0.5 * (self.vol**2))) / \
        ...
        if self.optType.lower() == 'call':
            ...
            optValue = self.strike * discount * norm.cdf(-d2) - self.stock * norm.cdf(-d1)
