...
class BacktestingInSample_MeanRevert:
    ...
    def Backtest(self, hurstLevel, hlLevelLower, hlLevelUpper, smaMult):
        countLoop = 0
        ...
        for i in range(len(self.data) - self.numCalibrationData):
            ...
            portCombinations = self.signal.StockSelection(history.iloc[:, 1:], self.numStocks, 'high')
            ...
            for key, value in portWeight.items():
                ...
                # Short
                if hurst < hurstLevel and hl > hlLevelLower and hl < hlLevelUpper:
                    if ((lastPrice > mu) and (longShortSignal['sma short'].iloc[-1] > mu + sigma * smaMult)) or \
                       ((lastPrice > mu) and (longShortSignal['sma short'].iloc[-1] < mu - sigma * smaMult)):
                        sell       = histData['total'].iloc[-1]
                        stopLoss   = sell * (1 + self.stopLossPerc1)
                        priceLimit = sell * (1 - self.profitLimit)
                        direction  = 'short'
                    
                    elif ((lastPrice < mu) and (longShortSignal['sma short'].iloc[-1] < mu - sigma * smaMult)) or \
                         ((lastPrice < mu) and (longShortSignal['sma short'].iloc[-1] > mu + sigma * smaMult)):
