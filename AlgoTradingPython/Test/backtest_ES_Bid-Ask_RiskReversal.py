...
def build_butterfly_database(
    df: pd.DataFrame,
    wings_each_side: int = 5,
    output_path: str | None = None
) -> pd.DataFrame:
    """
    Build a butterfly bid/ask database from option quote data.

    Expected input columns:
        date, time, futures, opt_type, strike, bid, ask

    Output columns:
        date, time,
        put ATM-5, ..., put ATM,
        call ATM, call ATM+1, ..., call ATM+5

    Each butterfly cell is a string like:
        '1.25~2.50'
    meaning:
        butterfly_bid ~ butterfly_ask
    """

    required_cols = {"date", "time", "futures", "opt_type", "strike", "bid", "ask"}
    ...
    data["opt_type"] = data["opt_type"].astype(str).str.lower().str.strip()
    ...
    data = data.dropna(subset=["date", "time", "opt_type", "strike", "bid", "ask", "futures"])
    data = data[data["opt_type"].isin(["call", "put"])]

    # Remove duplicates if any: keep last quote for same snapshot / type / strike
    data = (
        data.sort_values(["date", "time"])
            .drop_duplicates(subset=["date", "time", "opt_type", "strike"], keep="last")
    )

    # One futures value per timestamp
    fut_df = (
        data[["date", "time", "futures"]]
        .drop_duplicates(subset=["date", "time"], keep="last")
        .set_index(["date", "time"])
    )

    # Pivot call and put separately
    call_bid = (
        data[data["opt_type"] == "call"]
        .pivot(index=["date", "time"], columns="strike", values="bid")
        .sort_index(axis=1)
    )
    call_ask = (
        data[data["opt_type"] == "call"]
        .pivot(index=["date", "time"], columns="strike", values="ask")
        .sort_index(axis=1)
    )

    put_bid = (
        data[data["opt_type"] == "put"]
        .pivot(index=["date", "time"], columns="strike", values="bid")
        .sort_index(axis=1)
    )
    put_ask = (
    ...
    # Use common strike grid if possible
    all_strikes = sorted(set(data["strike"].dropna().unique()))
    if len(all_strikes) < 3:
        raise ValueError("Need at least 3 strikes to form a butterfly.")

    # Infer strike step from the full dataset
    diffs = np.diff(all_strikes)
    diffs = diffs[diffs > 0]
    if len(diffs) == 0:
        raise ValueError("Could not infer strike spacing.")
    strike_step = int(pd.Series(diffs).mode().iloc[0])

    # Reindex columns to full uniform grid
    full_strikes = np.arange(min(all_strikes), max(all_strikes) + strike_step, strike_step)

    call_bid = call_bid.reindex(columns=full_strikes)
    call_ask = call_ask.reindex(columns=full_strikes)
    put_bid = put_bid.reindex(columns=full_strikes)
    put_ask = put_ask.reindex(columns=full_strikes)

    # Align everything to same timestamp index
    common_index = fut_df.index
    call_bid = call_bid.reindex(common_index)
    call_ask = call_ask.reindex(common_index)
    put_bid = put_bid.reindex(common_index)
    put_ask = put_ask.reindex(common_index)

    def nearest_strike(x: float, strikes: np.ndarray) -> float:
        idx = np.abs(strikes - x).argmin()
        return strikes[idx]

    def format_bfly_cell(bid_val: float, ask_val: float) -> str:
        if pd.isna(bid_val) or pd.isna(ask_val):
            return ""
        return f"{bid_val:.2f}~{ask_val:.2f}"

    result_rows = []

    strikes_array = np.array(full_strikes)

    for (date, time), fut_row in fut_df.iterrows():
        fut = fut_row["futures"]
        atm_strike = nearest_strike(fut, strikes_array)
        ...
        # ---------- PUT butterflies ----------
        # put ATM-5 ... put ATM
        # centers = atm-5*step, ..., atm
        for j in range(wings_each_side, -1, -1):
            center = atm_strike - j * strike_step
            lower = center - strike_step
            upper = center + strike_step

            col_name = "put ATM" if j == 0 else f"put ATM-{j}"

            if lower in put_bid.columns and center in put_bid.columns and upper in put_bid.columns:
                b1 = put_bid.at[(date, time), lower]
                a2 = put_ask.at[(date, time), center]
                b3 = put_bid.at[(date, time), upper]

                a1 = put_ask.at[(date, time), lower]
                b2 = put_bid.at[(date, time), center]
                a3 = put_ask.at[(date, time), upper]

                bfly_bid = b1 - 2 * a2 + b3
                bfly_ask = a1 - 2 * b2 + a3

                row_out[col_name] = format_bfly_cell(bfly_bid, bfly_ask)
            else:
                row_out[col_name] = ""

        # ---------- CALL butterflies ----------
        # call ATM, call ATM+1, ..., call ATM+5
        for j in range(0, wings_each_side + 1):
            center = atm_strike + j * strike_step
            lower = center - strike_step
            upper = center + strike_step

            col_name = "call ATM" if j == 0 else f"call ATM+{j}"

            if lower in call_bid.columns and center in call_bid.columns and upper in call_bid.columns:
                b1 = call_bid.at[(date, time), lower]
                a2 = call_ask.at[(date, time), center]
                b3 = call_bid.at[(date, time), upper]

                a1 = call_ask.at[(date, time), lower]
                b2 = call_bid.at[(date, time), center]
                a3 = call_ask.at[(date, time), upper]
    ...
    if output_path:
        if output_path.endswith(".csv"):
            ...
            raise ValueError("output_path must end with .csv, .pkl, or .parquet")

    return result

butterfly_db = build_butterfly_database(
    data,
    wings_each_side=5)
